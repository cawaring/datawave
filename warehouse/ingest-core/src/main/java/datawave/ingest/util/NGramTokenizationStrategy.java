            ...
            // Otherwise, try to tokenize
                ...
                try {
                    ...
                    try (NGramTokenizer tokenizer = new NGramTokenizer(2, maxNGramLength)) {
                        ...
                        tokenizer.reset();
                        try (TokenStream tokenStream = new ClassicFilter(tokenizer)) {
                            try (TokenStream lcfTokenStream = new LowerCaseFilter(tokenStream)) {
                                lcfTokenStream.addAttribute(CharTermAttribute.class);
